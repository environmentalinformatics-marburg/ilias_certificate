...
imported.src = 'jszip/dist/jszip.js';
...
function exportToXML(filename) {
  ...
  xmlFile += '<fo:region-body margin="0cm 2cm 0cm 2cm"/><fo:region-before region-name="background-image" extent="0"/>';
	xmlFile += '</fo:simple-page-master></fo:layout-master-set>';
 	...
 	xmlFile += '<fo:block><fo:external-graphic src="url(background.jpg)" content-height="297mm" content-width="210mm"/></fo:block></fo:block-container></fo:static-content>';
  ...
  xmlFile += '<fo:block text-align="right" font-size="0.7em">Marburg, den [DATE]</fo:block>'; //DATE
	xmlFile += '<fo:block text-align="right" font-size="0.7em">Telefon: (0 64 21) 28-23043</fo:block>'; //Telefon
	xmlFile += '<fo:block text-align="right" font-size="0.7em">E-Mail: [COURSE_OWNER_MAIL]</fo:block>'; //E-Mail
	xmlFile += '<fo:block text-align="right" font-size="0.7em">Deutschhausstraße 3, 35032 Marburg</fo:block>'; //straße
	xmlFile += '</fo:block-container>'; // CONTAINER 2
  /********************************************************************************************************/
  xmlFile += '<fo:block text-align="left" font-size="0.6em">Hochschulrechenzentrum der Philipps-Universität Marburg</fo:block> '; //Workplace
	xmlFile += '<fo:block text-align="left" font-size="0.6em" border-bottom="1px solid black">Hans-Meerwein-Straße, 35032 Marburg</fo:block>'; //Workplace
  ...
  xmlFile += '<fo:block></fo:block>';
  xmlFile += '<fo:inline font-weight="bold" font-size="1.2em">Workshop: [COURSE_TITLE]</fo:inline>';
  xmlFile += '<fo:block></fo:block>';
  xmlFile += '<fo:block font-weight="bold">Inhalt des Workshops:</fo:block>';
  ...
  xmlFile = xmlFile.replace(/\[COURSE_TITLE]/gi, ufirst = document.forms[0].ctitle.value ); 
  xmlFile = xmlFile.replace(/\[COURSE_DESC]/gi, ufirst = document.forms[0].cinhalt.value );
...
var urls = ["images/background.jpg"];
var nombre = today.toISOString().substring(0, 10) + "_" + today.getUTCMilliseconds()+ "_course_certificate";
...
.then(function(content) {
    // see FileSaver.js
    saveAs(content, "export_certificate.zip");
});*/
